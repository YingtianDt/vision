...
LAYERS = ['Retina', 'LGN', 'V1', 'V2', 'V3', 'V4', 'LOC']  #, 'logits']


class BLTTemporalWrapper(PytorchWrapper):
    def forward(self, inputs): 
        # this function gets a list of preprocessed inputs and does the forward pass
        import torch
        tensor = torch.stack(inputs)
        tensor = tensor.to(self._device)
        return self._model.forward_vid(tensor)
    ...
    def get_activations(self, inputs, layer_names):
        ...
        with torch.no_grad():
            layer_results = self.forward(inputs)

        layer_results = OrderedDict({layer_name: layer_results[layer_name] for layer_name in layer_names})
        return layer_results

def get_model(model_name='blt_temporal', key_or_alias='blt_vs', image_size=224):
    """
    ...
    def transform_video(video):
        ...
        frames = torchvision_preprocess_sush(frames)
        return frames

    wrapper = BLTTemporalWrapper(model_name, blt_model, transform_video, 
                             process_output=process_output,
                             **inferencer_kwargs)
    return wrapper


def torchvision_preprocess_sush(img, normalize_mean=(0.5, 0.5, 0.5), normalize_std=(0.5, 0.5, 0.5)):
...
class BLT_VS(nn.Module):
    ...
    def forward_vid(
        ...
        bu_activations = [None for _ in self.areas]
        td_activations = [None for _ in self.areas]
        batch_size = vid_input.size(0)
        timesteps = list(range(vid_input.size(1)))
        layer_outputs = {}
        ...
        for idx, area in enumerate(self.areas):
            data = [activations[area][t]['bu'] for t in timesteps]
            data = torch.stack(data, dim=1).detach().cpu().numpy()
            layer_outputs[area] = data
